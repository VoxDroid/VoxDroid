...
export default function TerminalModal() {
  ...
  const [history, setHistory] = useState<Command[]>([
    {
      cmd: "welcome",
      output:
        "Welcome to VoxDroid Terminal v2.0. Type 'help' for available commands.",
    },
  ]);
  ...
  const handleTabCompletion = () => {
    ...
    const matches = availableDirs.filter((d) =>
      d.toLowerCase().startsWith(partial),
    );
    if (matches.length === 1) {
  ...
  const handleCommand = (e: React.FormEvent) => {
    ...
    switch (command) {
      case "help":
        output = (
          <div className="space-y-1">
            ...
            <p className="pl-4">
              <span className="text-yellow-400">cd &lt;dir&gt;</span> - Change
              directory (e.g., cd about, cd projects/Bytey)
            </p>
            <p className="pl-4">
              <span className="text-yellow-400">cd ..</span> - Go back / Go home
            </p>
            <p className="pl-4">
              <span className="text-yellow-400">ls</span> - List contents of
              current directory
            </p>
            <p className="pl-4">
              <span className="text-yellow-400">pwd</span> - Print working
              directory
            </p>
            <p className="mt-2 text-cyan-400 font-bold">Theme:</p>
            <p className="pl-4">
              <span className="text-yellow-400">theme dark</span> - Switch to
              dark mode
            </p>
            <p className="pl-4">
              <span className="text-yellow-400">theme light</span> - Switch to
              light mode
            </p>
            <p className="pl-4">
              <span className="text-yellow-400">theme toggle</span> - Toggle
              theme
            </p>
            <p className="mt-2 text-cyan-400 font-bold">Other:</p>
            <p className="pl-4">
              <span className="text-yellow-400">whoami</span> - Display current
              user
            </p>
            <p className="pl-4">
              <span className="text-yellow-400">neofetch</span> - System info
            </p>
            <p className="pl-4">
              <span className="text-yellow-400">cat README.md</span> - About
              this site
            </p>
            <p className="pl-4">
            ...
            {aiEnabled && (
              <p className="mt-2 text-gray-500 text-xs">
                AI: Use{" "}
                <span className="text-yellow-400">ai &lt;question&gt;</span> to
                ask the site assistant (requires an API key).
              </p>
            )}
          </div>
        );
      ...
      case "cd":
        if (!args || args === "~" || args === "/") {
          output = (
            <span className="text-green-400">Navigating to home...</span>
          );
          router.push("/");
          setCurrentDir("~");
        } else if (args === "..") {
          ...
          if (parts.length > 1) {
            ...
            output = (
              <span className="text-green-400">Navigating to /{parent}...</span>
            );
            router.push(`/${parent}`);
          ...
          const availableDirs = pages[baseDir] || pages["~"];
          const matchedDir = availableDirs.find(
            (d) => d.toLowerCase() === targetParts[0].toLowerCase(),
          );
          ...
          if (matchedDir && currentDir === "~") {
            ...
            output = (
              <span className="text-green-400">
                Navigating to /projects/{matchedDir}...
              </span>
            );
            ...
            const subDir = pages[pageDir]?.find(
              (d) => d.toLowerCase() === targetParts[1].toLowerCase(),
            );
            if (subDir) {
              output = (
                <span className="text-green-400">
                  Navigating to /{pageDir}/{subDir}...
                </span>
              );
              ...
              output = (
                <span className="text-red-400">
                  cd: no such directory: {args}
                </span>
              );
            }
          } else if (
            pages["~"].some((d) => d.toLowerCase() === args.toLowerCase())
          ) {
            // Going to a main page from anywhere
            const targetDir = pages["~"].find(
              (d) => d.toLowerCase() === args.toLowerCase(),
            )!;
            output = (
              <span className="text-green-400">
                Navigating to /{targetDir}...
              </span>
            );
        ...
        if (currentDir === "~" || !args) {
          output = (
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-x-4 gap-y-1">
              {(currentDir === "~"
                ? pages["~"]
                : pages[currentDir.split("/")[0]] || pages["~"]
              ).map((item, i) => (
      ...
      case "theme":
        if (args === "dark") {
          ...
          output = (
            <span className="text-green-400">Theme set to dark mode</span>
          );
        } else if (args === "light") {
          setTheme("light");
          localStorage.setItem("theme", "light");
          output = (
            <span className="text-green-400">Theme set to light mode</span>
          );
        } else if (args === "toggle") {
          ...
          output = (
            <span className="text-green-400">
              Theme toggled to {newTheme} mode
            </span>
          );
        } else {
          output = (
            <span className="text-gray-300">
              Current theme: {theme}. Usage: theme [dark|light|toggle]
            </span>
          );
        }
        break;
...
/_____/ `}
            ...
            <div className="space-y-0.5 text-sm">
              <p>
                ...
                <span className="text-cyan-400">voxdroid</span>
              </p>
              <p>-----------------</p>
              <p>
                <span className="text-cyan-400">OS:</span> VoxDroid Web v2.0
              </p>
              <p>
                <span className="text-cyan-400">Host:</span> voxdroid.vercel.app
              </p>
              <p>
                <span className="text-cyan-400">Shell:</span> vox-sh 1.0
              </p>
              <p>
                <span className="text-cyan-400">Theme:</span> {theme}
              </p>
              <p>
                <span className="text-cyan-400">Terminal:</span> VoxTerm
              </p>
              <p>
                <span className="text-cyan-400">Framework:</span> Next.js 16.1.1
              </p>
            </div>
          </div>
        );
        ...
        if (args === "readme.md" || args === "README.md") {
          output = (
            <div className="space-y-2 text-sm">
              ...
              <p>
                A Linux-themed developer portfolio built with Next.js,
              ...
              <p>
                Use `cd` to navigate, `ls` to list, `theme` to change
                appearance.
              </p>
            </div>
          );
      ...
      case "sudo":
        output = (
          <span className="text-red-500">
            ...
            Sorry, user guest is not allowed to execute &apos;
            {args || "command"}&apos; as root.
          </span>
        );
        break;
      ...
      case "rm":
        if (args.includes("-rf") || args.includes("*")) {
          ...
          output = (
            <span className="text-red-400">
              rm: cannot remove &apos;{args}&apos;: Permission denied
            </span>
          );
        }
        break;
      ...
      case "ai": {
        if (!aiEnabled) {
          output = (
            <span className="text-red-400">
              AI chat is currently disabled. Contact the administrator to enable
              this feature.
            </span>
          );
          break;
        ...
        if (!args) {
          output = (
            <span className="text-gray-300">
              Usage: ai &lt;your question&gt; — queries the site assistant
              (requires API key)
            </span>
          );
          break;
        ...
        output = (
          <span className="text-gray-400">
            Querying AI assistant… (streaming)
          </span>
        );
            ...
            if (!res.ok) {
              ...
              setHistory((h) => {
                ...
                copy[placeholderIndex] = {
                  cmd: rawInput,
                  output: <span className="text-red-400">{String(err)}</span>,
                };
                return copy;
              });
              return;
            ...
            if (!res.body) {
              setHistory((h) => {
                ...
                copy[placeholderIndex] = {
                  cmd: rawInput,
                  output: (
                    <span className="text-red-400">Empty response stream</span>
                  ),
                };
                return copy;
              });
    ...
    <AnimatePresence mode="wait">
      {isOpen && (
        ...
        >
          ...
          {/* Terminal Body */}
          <div
            className="flex-1 p-4 overflow-y-auto text-gray-300 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent"
