...
const project = {
  ...
  longDescription: `
  <p><strong>vox-hash</strong> is an open-source command-line interface (CLI) tool written in Rust for generating SHA1 and MD5 hashes and performing brute-force hash matching. Designed for security researchers, penetration testers, and developers, it provides a fast and flexible solution for hashing strings and cracking hashes using customizable charsets, wordlists, patterns, and rainbow tables. The tool leverages Rust's performance and parallelism (via <code>rayon</code>) to handle single and bulk operations efficiently, with progress bars (<code>indicatif</code>) and verbose logging for transparency.</p>
  <br>
  <p>The CLI supports five main commands: <code>enc</code> (hash a single string), <code>dec</code> (brute-force a single hash), bulk-enc (hash multiple strings), bulk-dec (brute-force multiple hashes), generate-table (create a rainbow table), and benchmark (measure hashing speed). With features like JSON output, concurrent processing, and pattern-based cracking, vox-hash is ideal for security testing and hash analysis. </p>
  `,
  image: "/project_images/VOXHASH.png",
  date: "May 2025",
  client: "Open Source Community",
  role: "Lead Developer",
  tags: ["Rust", "Hashing", "Command-line", "Regex", "Hash-bruteforce", "Rust-CLI"],
  ...
  "features": [
    "<strong>High-Performance Hashing:</strong> Utilizes Rust’s blazing speed and memory safety to provide ultra-fast SHA1 and MD5 hashing operations directly from the command line.",
    "<strong>Brute-Force Hash Matching:</strong> Includes a robust brute-force engine optimized for efficiently cracking SHA1 and MD5 hashes using customizable character sets and length settings.",
    "<strong>Zero-Dependency Binary:</strong> Distributes as a standalone Rust binary with no external runtime requirements, ensuring smooth operation on all major platforms.",
    "<strong>Cross-Platform CLI Tool:</strong> Fully compatible with Linux, macOS, and Windows, enabling seamless integration into any workflow.",
    "<strong>Easy-to-Use Interface:</strong> Features clear, concise command-line arguments and detailed help text for maximum usability regardless of experience level.",
    "<strong>Open Source & Extensible:</strong> Designed for the community—open-source under a permissive license, ready for customization, extension, and contributions."
  ],
  "challenges": [
  ],
  solutions: [
  ...
  "techStack": [
    { "name": "Rust", "description": "Core programming language for the CLI tool, offering high performance, safety, and modern concurrency." },
    { "name": "Cargo", "description": "Rust’s official package manager and build system, used to manage dependencies and produce optimized binaries." },
    { "name": "Clap", "description": "Rust library for parsing command-line arguments, enabling powerful and user-friendly CLI interfaces." },
    { "name": "SHA1 & MD5 Algorithms", "description": "Cryptographic hash algorithms implemented and used for hashing and verification tasks." },
    { "name": "Linux, macOS, Windows", "description": "The tool is built and tested for full compatibility across major operating systems." },
    { "name": "GitHub Actions", "description": "Continuous integration and testing pipeline for automated builds and quality assurance." },
    { "name": "GitHub", "description": "Platform for open-source hosting, version control, and collaborative development." }
  ]
}
