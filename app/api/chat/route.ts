  ...
  try {
    ...
    if (!aiEnabled) {
      return NextResponse.json(
        {
          error:
            "AI chat is currently disabled. Contact the administrator to enable this feature.",
        },
        { status: 403 },
      );
    }
    ...
    if (!apiKey) {
      return NextResponse.json(
        {
          error:
            "Server not configured. Set GOOGLE_GENERATIVE_AI_API_KEY in your environment.",
        },
        { status: 500 },
      );
    }
      ...
      projectsInfo = projects
        .map((project) => {
          const readmeExcerpt = project.readme
            ? project.readme.replace(/#+\s*/g, "").substring(0, 500) +
    ...
    if (!useSdk) {
      try {
        ...
        if (!r.ok) {
          ...
          return NextResponse.json(
            { error: "Model error (REST)", details },
            { status: 502 },
          );
        }
        const j = await r.json();
        const text =
          j?.candidates?.[0]?.output ?? j?.output?.[0]?.content ?? "";
        ...
        return NextResponse.json(
          { error: String(restErr?.message || restErr) },
          { status: 500 },
        );
      }
    }
    ...
    try {
      ...
      for (const m of modelCandidates) {
        try {
          ...
          if (
            result &&
            typeof (result as any).toDataStreamResponse === "function"
          ) {
          ...
          if (
            ...
            Symbol.asyncIterator in (result as any).textStream
          ) {
          ...
          if (result && typeof result === "object") {
            ...
            return new Response(String(maybeText), {
              headers: { "Content-Type": "text/plain; charset=utf-8" },
            });
          }
      ...
      return NextResponse.json(
        {
          error: "No streaming model available",
