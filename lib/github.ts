  ...
  try {
    ...
    for (const url of urls) {
      try {
        ...
        const longestStreakLength =
          typeof data.longestStreak === "object"
...
function calculateStreaksFromDays(
  ...
  for (let i = 0; i < sortedDays.length; i++) {
    ...
    const diffFromPrev = Math.floor(
      (prevDay.getTime() - dayDate.getTime()) / (1000 * 60 * 60 * 24),
    );
...
async function fetchLifetimeContributions(
  username: string,
  accountCreatedAt: string,
): Promise<number> {
  if (!GITHUB_TOKEN) {
    console.warn(
      "GITHUB_TOKEN not set - cannot fetch lifetime contributions via GraphQL",
    );
    return 0;
  }
  ...
  try {
    ...
    const totalLifetime = yearlyContributions.reduce(
      (sum, count) => sum + count,
      0,
    );

    console.log(
      `Lifetime contributions for ${username}: ${totalLifetime} (${currentYear - createdYear + 1} years)`,
    );
...
export async function getGitHubUserStats(
  ...
  try {
    ...
    const accountCreated = createdAt.toLocaleDateString("en-US", {
      month: "short",
      year: "numeric",
    });
    ...
    if (eventsResponse.ok) {
      ...
      if (sortedDates.length > 0) {
        ...
        const daysDiff = Math.floor(
          (now.getTime() - lastContrib.getTime()) / (24 * 60 * 60 * 1000),
        );
        if (daysDiff <= 1) {
          currentStreak = tempStreak;
        }
    ...
    return {
      ...
      currentStreak:
        ...
        currentStreak,
      currentStreakStart:
        graphqlStreak?.currentStreakStart ||
        streakStats?.currentStreakStart ||
        "",
      currentStreakEnd:
        graphqlStreak?.currentStreakEnd || streakStats?.currentStreakEnd || "",
      longestStreak:
        graphqlStreak?.longestStreak ??
        streakStats?.longestStreak ??
        longestStreak,
      longestStreakStart:
        graphqlStreak?.longestStreakStart ||
        streakStats?.longestStreakStart ||
        "",
      longestStreakEnd:
        graphqlStreak?.longestStreakEnd || streakStats?.longestStreakEnd || "",
      ...
      contributionsThisYear:
        streakStats?.totalContributions || contributionsThisYear,
      contributionGraph,
      recentActivity,
...
export async function getRepoCommits(
  ...
  const commits = await fetchGitHub<GitHubCommit[]>(
    `/repos/${fullName}/commits?per_page=${count}`,
  );
  return commits || [];
}
...
export async function getRepoContributors(
  ...
  const contributors = await fetchGitHub<GitHubContributor[]>(
    `/repos/${fullName}/contributors?per_page=10`,
  );
  return contributors || [];
}
...
export async function getRepoReleases(
  ...
  const releases = await fetchGitHub<GitHubRelease[]>(
    `/repos/${fullName}/releases?per_page=5`,
  );
  return releases || [];
}
...
export async function getRepoTree(
  ...
  try {
    ...
    const tree = await fetchGitHub<GitHubTree>(
...
export async function getFullProjectData(
  ...
  const [repo, readme, languages, commits, contributors, releases, fileTree] =
    await Promise.all([
  ...
  return {
    ...
    category:
      override.category || getCategoryFromLanguage(repo.language, repo.topics),
...
function getCategoryFromLanguage(
  language: string | null,
  topics: string[],
): string {
  const topicsLower = topics.map((t) => t.toLowerCase());

  if (topicsLower.some((t) => ["cli", "command-line", "terminal"].includes(t)))
    return "CLI Tool";
  if (
    topicsLower.some((t) => ["web", "website", "nextjs", "react"].includes(t))
  )
    return "Web App";
  if (
    topicsLower.some((t) => ["desktop", "gui", "pyqt", "electron"].includes(t))
  )
    return "Desktop App";
  if (topicsLower.some((t) => ["library", "package", "module"].includes(t)))
    return "Library";
  if (
    topicsLower.some((t) =>
      ["tutorial", "learning", "snippets", "examples"].includes(t),
    )
  )
    return "Educational";
...
export async function getAllProjects(): Promise<ProjectData[]> {
  ...
  const validProjects = projects.filter(
    (p): p is ProjectData => p !== null && p !== undefined,
  );
